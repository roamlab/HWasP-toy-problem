...
def run_task(snapshot_config, *_):
    ...
    with LocalTFRunner(snapshot_config=snapshot_config) as runner:
        # env = TfEnv(normalize(MassSpringEnv_OptL_MultiSegments_HwAsAction(params), normalize_action=False, normalize_obs=False, normalize_reward=True, reward_alpha=0.1))
        ...
        mech_policy_model = MechPolicyModel_OptL_MultiSegments_HwAsAction(params)

        policy = CompMechPolicy_OptL_MultiSegments_HwAsAction(name='comp_mech_policy', 
                env_spec=env.spec, 
...
if __name__=='__main__':
    ...
    run_experiment(run_task, exp_prefix='ppo_opt_l_multi_segments_hw_as_action_{}'.format(args.exp_id), snapshot_mode='last', seed=args.seed, force_cpu=True)